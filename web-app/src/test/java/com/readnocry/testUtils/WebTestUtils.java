...
public class WebTestUtils {
    ...
    public WebTestUtils(AppUserService appUserService, BookStockService bookStockService) {
    ...
    public BookMetaData createBookForAppUser(AppUser appUser) throws IOException {
        ...
        Mockito.when(Files.exists(Mockito.any())).thenReturn(false);
        Mockito.when(Files.createDirectories(Mockito.any())).thenReturn(null);
        Mockito.when(Files.copy(Mockito.any(InputStream.class), Mockito.any())).thenReturn(0L);
        Mockito.when(Files.newInputStream(Mockito.any()))
